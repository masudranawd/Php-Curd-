...
<?php /*fetch data for update module*/
	if(isset($_GET['upid'])){
		$upid = $_GET['upid']; 
...
?>
	<tr>
		...
		<td>
			<a href="" class="btn btn-info btn-sm" data-toggle="modal" data-target="#mobile_view<?php echo $data['id']; ?>">View</a>
			<a href="?page=Mobile&upid=<?php echo $data['id']; ?>" class="btn btn-warning btn-sm">Update</a>
			<a href="?page=Mobile&delid=<?php echo $data['id']; ?>" class="btn btn-danger btn-sm">Remove</a>
		</td>
	</tr>
<!-------------------------------------------------------------->
<!-- Modal -->
<div class="modal fade" id="mobile_view<?php echo $data['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel"><?php echo $data['company'].' &nbsp; '.$data['model']; ?></h4>
      </div>
      <div class="modal-body">
        <?php 
        	echo '<h2>Brand Name # '.$data['company'].'</h2>';
        	echo '<h2>Model # '.$data['model'].'</h2>';
        	echo '<h4>Price # '.$data['price'].'.tk</h4>';
        	echo 'Specification # '.$data['details']; 
        ?>
      </div>
